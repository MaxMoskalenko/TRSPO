...
def thread_function(lock, obj1, obj2, thread_index):
    ...
    for _ in range(k):
        lock.acquire()
        try:
            new_obj1_data = random.randint(
                *RANDOM_NUMBER_INTERVAL) + obj1.get()
            obj1.set(new_obj1_data)

            new_obj2_data = random.randint(
                *RANDOM_NUMBER_INTERVAL) + obj2.get()
...
if __name__ == "__main__":
    ...
    for i in range(n):
        new_th = threading.Thread(
            target=thread_function, args=(lock, obj1, obj2, i,)
        )
